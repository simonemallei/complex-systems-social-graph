...
def compute_update(G, n_update_list, wii_list=None):
    ...
    for idx, n_update in enumerate(n_update_list):
        ...
        weight_noose = (
            wii_list[idx]
            if wii_list is not None
            else beba_beta_list[idx] * opinions[n_update] * opinions[n_update] + 1
        )
        ...
        if op_den <= 0:
            new_opinions[n_update] = opinions[n_update] / abs(opinions[n_update])
        else:
            new_opinions[n_update] = op_num / op_den

        # Opinions are capped within [-1, 1]
        if new_opinions[n_update] < -1:
            new_opinions[n_update] = -1
        if new_opinions[n_update] > 1:
            new_opinions[n_update] = 1
...
def simulate_epoch_updated(G, rate_updating_nodes, wii_list=None):
    ...
    updating_nodes = np.random.choice(
        range(len(G.nodes())), size=n_updating_nodes, replace=False
    )
    try:
        G = compute_update(G, updating_nodes, wii_list)
    except LengthIsNotEqualError:
        print(
            "beta_list and/or wii list do not have the same length as the list of nodes to be updated"
        )
